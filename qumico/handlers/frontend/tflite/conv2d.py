...
@tflite_op("CONV_2D")
class CONV_2D(TFLiteBaseHandler):
    ...
    def create_onnx_node(cls, operator, inputs, outputs,
        ...
        node.onnx_value_infos.append(helper.make_tensor_value_info(input0_transpose_node_name,
                                                                   NP_TYPE_TO_TENSOR_TYPE[inputs[0].np_tensor_type],
                                                                   input_shape.tolist()))
        ...
        transposed_shape = (inputs[1].shape[0], inputs[1].shape[3], inputs[1].shape[1], inputs[1].shape[2])
        ...
        node.onnx_value_infos.append(helper.make_tensor_value_info(output0_transpose_node_name,
                                                                   NP_TYPE_TO_TENSOR_TYPE[outputs[0].np_tensor_type],
                                                                   output_shape.tolist()))  # <=

        node.onnx_value_infos.append(helper.make_tensor_value_info(output0_y_scale,
                                                                   TensorProto.FLOAT,
                                                                   (1,)))

        node.onnx_value_infos.append(helper.make_tensor_value_info(output0_y_zero_point,
                                                                   NP_TYPE_TO_TENSOR_TYPE[outputs[0].np_tensor_type],
                                                                   (1,)))

        # tensor
        # ToDo: add when no quant
        node.onnx_tensors.append(helper.make_tensor(input0_x_scale, TensorProto.FLOAT, (1,), input0_scale))
        node.onnx_tensors.append(
            helper.make_tensor(input0_x_zero_point, NP_TYPE_TO_TENSOR_TYPE[inputs[0].np_tensor_type], (1,),
                               input0_zero_point))
        node.onnx_tensors.append(helper.make_tensor(input1_x_scale, TensorProto.FLOAT, (1,), input1_scale))
        node.onnx_tensors.append(
            helper.make_tensor(input1_x_zero_point, NP_TYPE_TO_TENSOR_TYPE[inputs[1].np_tensor_type], (1,),
                               input1_zero_point))
        node.onnx_tensors.append(helper.make_tensor(output0_y_scale, TensorProto.FLOAT, (1,), output0_scale))
        node.onnx_tensors.append(
            helper.make_tensor(output0_y_zero_point, NP_TYPE_TO_TENSOR_TYPE[outputs[0].np_tensor_type], (1,),
                               output0_zero_point))

        # weight
        reshaped_buf = input_buffers[1].transpose((0, 3, 1, 2))
        node.onnx_tensors.append(helper.make_tensor(input1_name,
                                                    NP_TYPE_TO_TENSOR_TYPE[inputs[1].np_tensor_type],
                                                    reshaped_buf.shape,
                                                    reshaped_buf.tobytes()))
        ...
        if bias:
            input.append(input_names[2])

        node.onnx_nodes.append(helper.make_node("QLinearConv",
                                                inputs=input,
