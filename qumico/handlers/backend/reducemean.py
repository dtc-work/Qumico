...
class ReduceMean(ReductionMixin, BackendHandler):
    ...
    def generate_c_code_div(self):
        ...
        mean_div =  np.prod([input.shape[i] for i in axes])
        ...
        return TemplateArrayLoop.replace('[statements]',  ''.join(indent) + 'output' + output_dim + '/='+ str(mean_div) +';')
    ...
    def generate_c_code(self, **kwargs):
        ...
        mappingf.update({'mean_statements':self.generate_c_code_div()})
        res += '\n\n'
