...
"""
void op_Gemm( const op_Gemm_param *param, const void **inputs, void **outputs ) {
    const TYPE_A *A = *(inputs);
    const TYPE_B *B = *(inputs +1);
    const TYPE_C *C = *(inputs +2);
    TYPE_Y *Y       = *(outputs);
    ...
    INT64   A_m = param->i0_0max;
    INT64   A_k = param->i0_1max;
    INT64   B_k = param->i1_0max;
    INT64   B_n = param->i1_1max;
    INT64   C_m = param->i2_0max;
    INT64   C_n = param->i2_1max; 
    INT64   Y_m = param->o0_0max;
    INT64   Y_n = param->o0_1max;
    ...
    memset( (void *)Y, 0.0, sizeof(Y) * Y_m * Y_n );
    ...
    for (m=0; m < C_m; m++) {
        for (n=0; n < C_n; n++) {
            if (transA == 0) {
                if (transB == 0) {
                    for (k=0; k < B_k; k++) {
                        *(Y + (m * C_n) +n) += *(A + (m * A_k) +k) * *(B + (k * B_n) +n);
                    }
                    *(Y + (m * Y_n) +n) *= alpha;
                    if ((m < C_m) && (n < C_n)) {
                        *(Y + (m * Y_n) +n) += beta * *(C + (m * C_n) +n);
                    }
                } else {  // transB != 0
                    for (k=0; k < B_k; k++) {
                        *(Y + (m * C_n) +n) += *(A + (m * A_k) +k) * *(B + (n * B_k) +k);
                    }
                    *(Y + (m * Y_n) +n) *= alpha;
                    if ((m < C_m) && (n < C_n)) {
                        *(Y + (m * Y_n) +n) += beta * *(C + (m * C_n) +n);
