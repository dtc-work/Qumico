...
class AveragePool(BackendHandler):
    ...
    def instantiate(cls, node, **kwargs):
        ...
        if (attrs.get('ceil_mode') == None):       # define ceil_mode after AveragePool-10. default is 0.
            attrs['ceil_mode'] = 0
    ...
    # NOTSET: output_spatial_shape[i] = floor((input_spatial_shape[i] + pad_shape[i] - kernel_spatial_shape[i]) / strides_spatial_shape[i] + 1)
            ...
            if (auto_pad == 'SAME_UPPER') or (auto_pad == 'SAME_LOWER'):
                ...
                if(attrs['ceil_mode'] == 0):
                    ...
                    tmp_shape.append(math.ceil((input_data1.shape[-1] + attrs['pads'][0] + attrs['pads'][-1] - attrs['kernel_shape'][-1])/attrs['strides'][-1] +1))
    ...
    # NOTSET: output_spatial_shape[i] = floor((input_spatial_shape[i] + pad_shape[i] - kernel_spatial_shape[i]) / strides_spatial_shape[i] + 1)
            ...
            if (auto_pad == 'SAME_UPPER') or (auto_pad == 'SAME_LOWER'):
                ...
                if(attrs['ceil_mode'] == 0):
                    ...
                    tmp_shape.append(math.ceil((input_data1.shape[-2] + attrs['pads'][0] + attrs['pads'][-2] - attrs['kernel_shape'][-2])/attrs['strides'][-2] +1))
                    tmp_shape.append(math.ceil((input_data1.shape[-1] + attrs['pads'][1] + attrs['pads'][-1] - attrs['kernel_shape'][-1])/attrs['strides'][-1] +1))
    ...
    # NOTSET: output_spatial_shape[i] = floor((input_spatial_shape[i] + pad_shape[i] - kernel_spatial_shape[i]) / strides_spatial_shape[i] + 1)
            ...
            if (auto_pad == 'SAME_UPPER') or (auto_pad == 'SAME_LOWER'):
                ...
                if(attrs['ceil_mode'] == 0):
                    ...
                    tmp_shape.append(math.ceil((input_data1.shape[-3] + attrs['pads'][0] + attrs['pads'][-3] - attrs['kernel_shape'][-3])/attrs['strides'][-3] +1))
                    tmp_shape.append(math.ceil((input_data1.shape[-2] + attrs['pads'][1] + attrs['pads'][-2] - attrs['kernel_shape'][-2])/attrs['strides'][-2] +1))
                    tmp_shape.append(math.ceil((input_data1.shape[-1] + attrs['pads'][2] + attrs['pads'][-1] - attrs['kernel_shape'][-1])/attrs['strides'][-1] +1))
        else:
            raise(ValueError)
