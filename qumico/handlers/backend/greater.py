...
@onnx_op('Greater')
class Greater(ArithmeticMixin, BackendHandler):
    
    @classmethod
    def instantiate(cls, node, **kwargs):
        
        A, B = node.input_tensor_values
        c = A + B # if not broadcasable, numpy throw exception

        output_value = {node.valid_var_name(node.outputs[0]): 
                        np.ones(shape=c.shape, dtype=np.bool)}
    ...
    def generate_c_code(self, **kwargs):
        ...
        for element_num_x, element_num_y, step in zip_longest(X.shape[::-1], 
                                                              Y.shape[::-1], 
            ...
            if self.input_tensor_ndims[1] == 0:
                YStatementDims = '[0]'
        ...
        void {op_func_name}(void *op_param,{Atype} {A}{ADims} , {Btype} {B}{BDims}, {Ctype} {C}{CDims}, void *inputs_params, void* outputs_params)
        {{
        {statements}
        }}
        ''')
        ...
        mappingf.update({'ADims':c_helper.generate_dim_bracket(self.input_tensor_shapes[0])})
        mappingf.update({'BDims':c_helper.generate_dim_bracket(self.input_tensor_shapes[1])})
        mappingf.update({'CDims': c_helper.generate_dim_bracket(self.output_tensor_shapes[0])})

        mappingf.update({'Atype': data_type.np2c(self.input_tensor_dtypes[0])})
        mappingf.update({'Btype': data_type.np2c(self.input_tensor_dtypes[1])})
        mappingf.update({'Ctype': data_type.np2c(self.output_tensor_dtypes[0])})
