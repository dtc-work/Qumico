...
@onnx_op('QLinearConv')
class QLinearConv(BackendHandler):
    ...
    def instantiate(cls, node, **kwargs):
        ...
        if (len(node.input_tensor) == 9):  # Bias
            input_data3 = node.input_tensor[8]
        attrs = node.attrs
...
#                cls.PARALLEL_SIZE = outputs_shape[1]//attrs['group']
    ...
    def get_param_type_name(cls):
        return 'QlinearConvOpParam'


    @classmethod
    def get_c_op_file_name(cls):
        return ['qlinearconv.c']
    ...
    def get_op_variale_def(cls):
        ...
        res += '#define qlinearconv_CLAMP(x, low, high) ((x) > (high) ? (high) : ((x) < (low) ? (low) : (x)))'
...
# define parameters (on python)
        if (ndim == 3):
            ...
            X_scale = self.input_tensor_values[1][0]
            X_zero_point = self.input_tensor_values[2][0]
            W_dm = self.input_tensor_shapes[3][0]
            W_dc = self.input_tensor_shapes[3][1]
            W_dkd = 1
            W_dkh = 1
            W_dkw = self.input_tensor_shapes[3][2]
            W_scale = self.input_tensor_values[4][0]
            W_scale_len = self.input_tensor_shapes[4][0]
            W_zero_point = self.input_tensor_values[5][0]
            W_zero_point_len = self.input_tensor_shapes[5][0]
            Y_scale = self.input_tensor_values[6][0]
            Y_zero_point = self.input_tensor_values[7][0]
            ...
            if (len(self.input_tensor) == 9):  # with Bias
            ...
            W_dc = self.input_tensor_shapes[3][1]
            W_dkd = 1
            W_dkh = self.input_tensor_shapes[3][2]
            W_dkw = self.input_tensor_shapes[3][3]
            W_scale = self.input_tensor_values[4][0]
            W_scale_len = self.input_tensor_shapes[4][0]
            W_zero_point = self.input_tensor_values[5][0]
            W_zero_point_len = self.input_tensor_shapes[5][0]
            ...
            W_dm = self.input_tensor_shapes[3][0]
            W_dc = self.input_tensor_shapes[3][1]
            W_dkd = self.input_tensor_shapes[3][2]
            W_dkh = self.input_tensor_shapes[3][3]
            W_dkw = self.input_tensor_shapes[3][4]
            W_scale = self.input_tensor_values[4][0]
            W_scale_len = self.input_tensor_shapes[4][0]
            W_zero_point = self.input_tensor_values[5][0]
            W_zero_point_len = self.input_tensor_shapes[5][0]
...
#pragma omp parallel for
                        for (ic=0; ic<X_c; ic++) {{
                            for (d=0; d<X_d; d++) {{
                                for (h=0; h<X_h; h++) {{
                                    for (w=0; w<X_w; w++) {{
                                        work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, d, X_d, h, X_h, w, X_w)] = *(_X_pt + mat_idx5(n, X_n, ic, X_c, d, X_d, h, X_h, w, X_w)) - X_zero_point[0];
//                                        work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, d, X_d, h, X_h, w, X_w)] = *(_X_pt + mat_idx5(n, X_n, ic, X_c, d, X_d, h, X_h, w, X_w));
                                    }}
                                }}
                            }}
                        }}
...
#pragma omp parallel for
                ...
                if (W_zero_point_len == 1): # W_zero_point is scalar
                    ...
                    TemplateStatements += '''
                                        work_pad_w[mat_idx6(oc1, (aligned_Y_c/SIMD_VECTOR_SIZE), ic, aligned_X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, oc2, SIMD_VECTOR_SIZE)] = *(_W_pt + mat_idx5( (oc1*SIMD_VECTOR_SIZE+oc2), Y_c, ic/group, X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)) - W_zero_point[oc1*SIMD_VECTOR_SIZE];
                    '''
                TemplateStatements += '''
                                    }}
                                }}
                                        ...
                                        for (kd=kernel_shape_d_min; kd<kernel_shape_d_max; kd++) {{
                                            ...
                                            for (kh=kernel_shape_h_min; kh<kernel_shape_h_max; kh++) {{
                                                ...
                                                for (kw=kernel_shape_w_min; kw<kernel_shape_w_max; kw++) {{
                                                    ...
                                                    work_pad_o[mat_idx6( n, Y_n, oc1, Y_c/SIMD_VECTOR_SIZE, d, Y_d, h, Y_h, w, Y_w, oc2, SIMD_VECTOR_SIZE )] += work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, current_d, X_d, current_h, X_h, current_w, X_w)]
                                                                        * work_pad_w[mat_idx6(oc1, Y_c/SIMD_VECTOR_SIZE, ic, aligned_X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, oc2, SIMD_VECTOR_SIZE)];
//                                                    x_zero_point_shift += work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, current_d, X_d, current_h, X_h, current_w, X_w)];
//                                                    w_zero_point_shift += work_pad_w[mat_idx6(oc1, Y_c/SIMD_VECTOR_SIZE, ic, aligned_X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, oc2, SIMD_VECTOR_SIZE)];
//                                                    kernel_counter++;
                                                }}
                                            }}
                                        }}
                    ...
                    if (W_zero_point_len == 1): # W_zero_point is scalar
                        TemplateStatements += '''
//                                        work_pad_o[mat_idx6( n, Y_n, oc1, Y_c/SIMD_VECTOR_SIZE, d, Y_d, h, Y_h, w, Y_w, oc2, SIMD_VECTOR_SIZE )] += - X_zero_point[0] * w_zero_point_shift
...
//                                                                                                            + X_zero_point[0] * W_zero_point[0] * kernel_counter;
                        ...
                        TemplateStatements += '''
//                                        work_pad_o[mat_idx6( n, Y_n, oc1, Y_c/SIMD_VECTOR_SIZE, d, Y_d, h, Y_h, w, Y_w, oc2, SIMD_VECTOR_SIZE )] += - X_zero_point[0] * w_zero_point_shift
//                                                                                                            - W_zero_point[oc1*SIMD_VECTOR_SIZE+oc2] * x_zero_point_shift
                                    ...
                                    for (kd=kernel_shape_d_min; kd<kernel_shape_d_max; kd++) {{
                                        ...
                                        for (kh=kernel_shape_h_min; kh<kernel_shape_h_max; kh++) {{
                                            ...
                                            for (kw=kernel_shape_w_min; kw<kernel_shape_w_max; kw++) {{
                                                current_w = w*stride_w+kw*dilation_w-pad_w_begin;
                                                if (current_w<0 || current_w>=X_w) {{ continue; }}
                                                work_pad_o[mat_idx6( n, Y_n, oc1, Y_c/SIMD_VECTOR_SIZE, d, Y_d, h, Y_h, w, Y_w, oc2, SIMD_VECTOR_SIZE )] += work_pad_i[mat_idx5(n, X_n, (oc1*SIMD_VECTOR_SIZE+oc2), aligned_X_c, current_d, X_d, current_h, X_h, current_w, X_w)]
                                                                    * work_pad_w[mat_idx6(oc1, Y_c/SIMD_VECTOR_SIZE, 0, 1, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, oc2, SIMD_VECTOR_SIZE)];
//                                                x_zero_point_shift += work_pad_i[mat_idx5(n, X_n, (oc1*SIMD_VECTOR_SIZE+oc2), aligned_X_c, current_d, X_d, current_h, X_h, current_w, X_w)];
//                                                w_zero_point_shift += work_pad_w[mat_idx6(oc1, Y_c/SIMD_VECTOR_SIZE, (oc1*SIMD_VECTOR_SIZE+oc2)/group, aligned_X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, oc2, SIMD_VECTOR_SIZE)];
//                                                kernel_counter++;
                                            }}
                                        }}
                                    }}
                                            ...
                                            for (kd=kernel_shape_d_min; kd<kernel_shape_d_max; kd++) {{
                                                ...
                                                for (kh=kernel_shape_h_min; kh<kernel_shape_h_max; kh++) {{
                                                    ...
                                                    for (kw=kernel_shape_w_min; kw<kernel_shape_w_max; kw++) {{
                                                        current_w = w*stride_w+kw*dilation_w-pad_w_begin;
                                                        if (current_w<0 || current_w>=X_w) {{ continue; }}
                                                        work_pad_o[mat_idx6( n, Y_n, current_group*Y_c/group+oc1, Y_c/SIMD_VECTOR_SIZE, d, Y_d, h, Y_h, w, Y_w, oc2, SIMD_VECTOR_SIZE )] += work_pad_i[mat_idx5(n, X_n, current_group*aligned_X_c/group+ic, aligned_X_c/group, current_d, X_d, current_h, X_h, current_w, X_w)]
                                                                            * work_pad_w[mat_idx6(oc1, Y_c/SIMD_VECTOR_SIZE, ic, aligned_X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, oc2, SIMD_VECTOR_SIZE)];
//                                                        x_zero_point_shift += work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, current_d, X_d, current_h, X_h, current_w, X_w)];
//                                                        w_zero_point_shift += work_pad_w[mat_idx6(oc1, Y_c/SIMD_VECTOR_SIZE, ic, aligned_X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, oc2, SIMD_VECTOR_SIZE)];
//                                                        kernel_counter++;
...
#elif TRANSPOSE == 3  // no pad M_SEPARATE
                for (n=0; n<Y_n; n++) {{
#pragma omp parallel for
                    for (oc1=0; oc1<Y_c/SIMD_VECTOR_SIZE; oc1++) {{
                        for (oc2=0; oc2<SIMD_VECTOR_SIZE; oc2++) {{
...
#undef TRANSPOSE
        ...
        if (len(self.input_tensor) == 9):    # with Bias
            ...
            void {op_func_name}(void *op_param, {t} X{dims_X}, float X_scale[], {t} X_zero_point[], {t} W{dims_W}, float W_scale{dims_W_scale}, {t} W_zero_point{dims_W_zero_point}, float Y_scale[], {t} Y_zero_point[], int B{dims_B}, {t} Y{dims}, void *inputs_params, void* outputs_params)
            {{
                {statements}
            }}
            ''')
            ...
            TemplateFunction += cleandoc('''
            void {op_func_name}(void *op_param, {t} X{dims_X}, float X_scale[], {t} X_zero_point[], {t} W{dims_W}, float W_scale{dims_W_scale}, {t} W_zero_point{dims_W_zero_point}, float Y_scale[], {t} Y_zero_point[], {t} Y{dims}, void *inputs_params, void* outputs_params)
            {{
                {statements}
            }}
            ''')
        ...
        mappingf.update({'W': self.input_tensor_names[3]})
        mappingf.update({'dims_W': c_helper.generate_dim_bracket(self.input_tensor_shapes[3])}) 
        mappingf.update({'dims_W_scale': c_helper.generate_dim_bracket(self.input_tensor_shapes[4])})
        mappingf.update({'dims_W_zero_point': c_helper.generate_dim_bracket(self.input_tensor_shapes[5])})
        if (len(self.input_tensor) == 9):     # with Bias
            mappingf.update({'B': self.input_tensor_names[8]})
            mappingf.update({'dims_B': c_helper.generate_dim_bracket(self.input_tensor_shapes[8])}) 
