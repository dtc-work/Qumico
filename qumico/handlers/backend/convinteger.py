...
@onnx_op('ConvInteger')
class ConvInteger(BackendHandler):
...
#                work_pad_w = np.prod(node.input_tensor[3].shape)                
                work_pad_w = (node.input_tensor[3].shape[0] *
                              node.input_tensor[3].shape[1] *
                              math.ceil(node.input_tensor[3].shape[2]/cls.PARALLEL_SIZE)*cls.PARALLEL_SIZE)
...
#                work_pad_w = np.prod(node.input_tensor[3].shape)                
                work_pad_w = (node.input_tensor[3].shape[0] *
                              node.input_tensor[3].shape[1] *
                              math.ceil(node.input_tensor[3].shape[2]/cls.PARALLEL_SIZE)*cls.PARALLEL_SIZE *
                              math.ceil(node.input_tensor[3].shape[3]/cls.PARALLEL_SIZE)*cls.PARALLEL_SIZE *
                              math.ceil(node.input_tensor[3].shape[4]/cls.PARALLEL_SIZE)*cls.PARALLEL_SIZE)
                work_pad_o = (input_data1.shape[0] *
                             math.ceil(input_data2.shape[0]/cls.PARALLEL_SIZE)*cls.PARALLEL_SIZE *
...
#                cls.PARALLEL_SIZE = outputs_shape[1]//attrs['group']
    ...
    def get_param_type_name(cls):
        return 'ConvIntegerOpParam'


    @classmethod
    def get_c_op_file_name(cls):
        return ['convinteger.c']
    ...
    def get_op_variale_def(cls):
        ...
        if (cls.SIMD):
            ...
            res += '#define ALIGN_SIZE (sizeof(int) * SIMD_VECTOR_SIZE)' + '\n'
            res += '#endif' + '\n'
            res += '\n'
            res += '__attribute__ ((aligned(ALIGN_SIZE))) int work_pad_i[{}];'.format(str(cls.WORK_PAD_I_SIZE)) + '\n'
            res += '__attribute__ ((aligned(ALIGN_SIZE))) int work_pad_w[{}];'.format(str(cls.WORK_PAD_W_SIZE)) + '\n'
            res += '__attribute__ ((aligned(ALIGN_SIZE))) int work_pad_o[{}];'.format(str(cls.WORK_PAD_O_SIZE)) + '\n'

        res += '#ifndef SINGLE_ALIGN_SIZE' + '\n'
        res += '#define SINGLE_ALIGN_SIZE (sizeof(int))' + '\n'
        res += '#endif' + '\n'
        res += '__attribute__ ((aligned(SINGLE_ALIGN_SIZE))) int work_pad_int[{}];'.format(str(cls.WORK_PAD_O_SIZE)) + '\n'
        ...
        res += '#define convinteger_CLAMP(x, low, high) (x)'
#        res += '#define convinteger_CLAMP(x, low, high) ((x) > (high) ? (high) : ((x) < (low) ? (low) : (x)))'
...
# define parameters (on python)
        if (ndim == 3):
            ...
            if (len(self.input_tensor) >= 3):
                X_zero_point = self.input_tensor_values[2][0]
            else:
                X_zero_point = [0,]
            ...
            if (len(self.input_tensor) >= 4):
                W_zero_point = self.input_tensor_values[3][0]
                W_zero_point_len = self.input_tensor_shapes[3][0]
            else:
                W_zero_point = (0,)
                W_zero_point_len = 1
            Y_scale = 1
...
# define parameters (on C)
        if (ndim == 3):
            ...
            TemplateStatements = '''
            {t_in}* _X_pt = &X[0][0][0][0];
            {t_in}* _W_pt = &W[0][0][0][0];
            {t_out}* _Y_pt = &Y[0][0][0][0];
            '''
        elif (ndim == 5):
            TemplateStatements = '''
            {t_in}* _X_pt = &X[0][0][0][0][0];
            {t_in}* _W_pt = &W[0][0][0][0][0];
            {t_out}* _Y_pt = &Y[0][0][0][0][0];
            '''
        ...
        TemplateStatements += '''
            ...
            const int  X_scale[] = {{1}};
        '''
        if (len(self.input_tensor) < 3):    # zero_point
            TemplateStatements += '''
        ...
        TemplateStatements += '''
            ...
            const int  W_zero_point_len = {W_zero_point_len};
            const int  W_scale[] = {W_scale_elements};
        '''
        if (len(self.input_tensor) < 4):    # zero_point
            TemplateStatements += '''
            const int  W_zero_point[] = {W_zero_point_elements};
            '''
        TemplateStatements += '''
            const int  Y_n = {Y_dn};
            const int  Y_c = {Y_dc};
...
#ifdef TRANSPOSE
memset( (void *)work_pad_i, 0, sizeof(int) * X_n * aligned_X_c * (padded_X_d) * (padded_X_h) * (padded_X_w) );
#if TRANSPOSE == 1
                    for (n=0; n<X_n; n++) {{
#pragma omp parallel for
                        for (d=0; d<X_d; d++) {{
                            for (h=0; h<X_h; h++) {{
                                for (w=0; w<X_w; w++) {{
                                    for (ic=0; ic<X_c; ic++) {{
                                        work_pad_i[mat_idx5(n, X_n, (d+pad_d_begin), (padded_X_d), (h+pad_h_begin), (padded_X_h), (w+pad_w_begin), (padded_X_w), ic, aligned_X_c)] = *(_X_pt + mat_idx5(n, X_n, ic, X_c, d, X_d, h, X_h, w, X_w)) - X_zero_point[0];
                                    }}
                                }}
                            }}
                        }}
                                ...
                                for (h=0; h<X_h; h++) {{
                                    for (w=0; w<X_w; w++) {{
                                        for (oc2=0; oc2<SIMD_VECTOR_SIZE; oc2++) {{
                                            work_pad_i[mat_idx6(n, X_n, oc1, aligned_X_c/SIMD_VECTOR_SIZE, (d+pad_d_begin), (padded_X_d), (h+pad_h_begin), (padded_X_h), (w+pad_w_begin), (padded_X_w), oc2, SIMD_VECTOR_SIZE)] = *(_X_pt + mat_idx5(n, X_n, (oc1*SIMD_VECTOR_SIZE+oc2), X_c, d, X_d, h, X_h, w, X_w)) - X_zero_point[0];
                                        }}
                                    }}
                                }}
                            }}
...
#pragma omp parallel for
                        for (ic=0; ic<X_c; ic++) {{
                            for (d=0; d<X_d; d++) {{
                                for (h=0; h<X_h; h++) {{
                                    for (w=0; w<X_w; w++) {{
                                        work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, (d+pad_d_begin), (padded_X_d), (h+pad_h_begin), (padded_X_h), (w+pad_w_begin), (padded_X_w))] = *(_X_pt + mat_idx5(n, X_n, ic, X_c, d, X_d, h, X_h, w, X_w)) - X_zero_point[0];
                                    }}
                                }}
                            }}
                        }}
...
#pragma omp parallel for
                ...
                '''
                if (W_zero_point_len == 1): # W_zero_point is scalar
                    TemplateStatements += '''
                                    work_pad_w[mat_idx5(kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, ic, aligned_X_c/group, oc, aligned_Y_c)] = *(_W_pt + mat_idx5( oc, Y_c, ic/group, X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w )) - W_zero_point[0];
                    '''
                else: # W_zero_point is 1D-array
                    TemplateStatements += '''
                                    work_pad_w[mat_idx5(kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, ic, aligned_X_c/group, oc, aligned_Y_c)] = *(_W_pt + mat_idx5( oc, Y_c, ic/group, X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w )) - W_zero_point[oc];
                    '''
                TemplateStatements += '''
                                }}
                            }}
...
#pragma omp parallel for
                ...
                if (W_zero_point_len == 1): # W_zero_point is scalar
                    TemplateStatements += '''
                                        work_pad_w[mat_idx6(oc1, (aligned_Y_c/SIMD_VECTOR_SIZE), ic, aligned_X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, oc2, SIMD_VECTOR_SIZE)] = *(_W_pt + mat_idx5( (oc1*SIMD_VECTOR_SIZE+oc2), Y_c, ic/group, X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)) - W_zero_point[0];
                    '''
                else: # W_zero_point is 1D-array
                    TemplateStatements += '''
                                        work_pad_w[mat_idx6(oc1, (aligned_Y_c/SIMD_VECTOR_SIZE), ic, aligned_X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w, oc2, SIMD_VECTOR_SIZE)] = *(_W_pt + mat_idx5( (oc1*SIMD_VECTOR_SIZE+oc2), Y_c, ic/group, X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)) - W_zero_point[oc1*SIMD_VECTOR_SIZE+oc2];
                    '''
                TemplateStatements += '''
                                    }}
                                }}
...
// bias settings
                ...
                if (B_d0 == 0):
                    TemplateStatements += '''
            memset( (void *)work_pad_o, 0, sizeof(int) * Y_n * Y_c * Y_d * Y_h * Y_w );
                    '''
                TemplateStatements += '''
            for (n=0; n<Y_n; n++) {{
                '''
                    ...
                    for kd in list(range(kernel_shape_d)):
                        for kh in list(range(kernel_shape_h)):
                            for kw in list(range(kernel_shape_w)):
                                ...
                                TemplateStatements += '''
                                    work_pad_o[mat_idx5(n, Y_n, d, Y_d, h, Y_h, w, Y_w, oc, Y_c)] += work_pad_i[mat_idx5(n, X_n, (d*stride_d+{kd}), padded_X_d, (h*stride_h+{kh}), padded_X_h, (w*stride_w+{kw}), padded_X_w, ic, aligned_X_c)] * work_pad_w[mat_idx5({kd}, kernel_shape_d, {kh}, kernel_shape_h, {kw}, kernel_shape_w, ic, aligned_X_c, oc, Y_c)];
...
#pragma omp parallel for
                ...
                if (W_scale_len == 1): # W_scale is scalar
                    TemplateStatements += '''
                                    *(_Y_pt + mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_o[mat_idx5(n, Y_n, d, Y_d, h, Y_h, w, Y_w, oc, aligned_Y_c)] * X_scale[0] * W_scale[0] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                    '''
                else: # W_scale is 1D-array
                    TemplateStatements += '''
                                    *(_Y_pt + mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_o[mat_idx5(n, Y_n, d, Y_d, h, Y_h, w, Y_w, oc, aligned_Y_c)] * X_scale[0] * W_scale[oc] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                    '''
                TemplateStatements += '''
                                }}
                            }}
...
#pragma omp parallel for
                ...
                if (W_scale_len == 1): # W_scale is scalar
                    TemplateStatements += '''
                                        *(_Y_pt + mat_idx5(n, Y_n, (oc1*SIMD_VECTOR_SIZE+oc2), Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_o[mat_idx6(n, Y_n, oc1, (Y_c/SIMD_VECTOR_SIZE), d, Y_d, h, Y_h, w, Y_w, oc2, SIMD_VECTOR_SIZE)] * X_scale[0] * W_scale[0] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                    '''
                else: # W_scale is 1D-array
                    TemplateStatements += '''
                                        *(_Y_pt + mat_idx5(n, Y_n, (oc1*SIMD_VECTOR_SIZE+oc2), Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_o[mat_idx6(n, Y_n, oc1, (Y_c/SIMD_VECTOR_SIZE), d, Y_d, h, Y_h, w, Y_w, oc2, SIMD_VECTOR_SIZE)] * X_scale[0] * W_scale[oc1*SIMD_VECTOR_SIZE+oc2] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                    '''
                TemplateStatements += '''
                                    }}
                                }}
                ...
                for (n=0; n<X_n; n++) {{
                    for (ic=0; ic<X_c; ic++) {{
                        for (d=0; d<pad_d_begin; d++) {{
                            for (h=0; h<padded_X_h; h++) {{
                                for (w=0; w<padded_X_w; w++) {{
                                    work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, d, padded_X_d, h, padded_X_h, w, padded_X_w)] = 0;
                                }}
                            }}
                        }}
                        for (d=0; d<X_d; d++) {{
                            for (h=0; h<pad_h_begin; h++) {{
                                for (w=0; w<padded_X_w; w++) {{
                                    work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, (d+pad_d_begin), padded_X_d, h, padded_X_h, w, padded_X_w)] = 0;
                                }}
                            }}
                            for (h=0; h<X_h; h++) {{
                                for (w=0; w<pad_w_begin; w++) {{
                                    work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, (d+pad_d_begin), padded_X_d, (h+pad_h_begin), padded_X_h, w, padded_X_w)] = 0;
                                }}
                                for (w=0; w<X_w; w++) {{
                                    work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, (d+pad_d_begin), padded_X_d, (h+pad_h_begin), padded_X_h, (w+pad_w_begin), padded_X_w)] = *(_X_pt + mat_idx5(n, X_n, ic, X_c, d, X_d, h, X_h, w, X_w)) - X_zero_point[0];
                                }}
                                for (w=0; w<pad_w_end; w++) {{
                                    work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, (d+pad_d_begin), padded_X_d, (h+pad_h_begin), padded_X_h, (w+pad_w_begin+X_w), padded_X_w)] = 0;
                                }}
                            }}
                            for (h=0; h<pad_h_end; h++) {{
                                for (w=0; w<padded_X_w; w++) {{
                                    work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, (d+pad_d_begin), padded_X_d, (h+pad_h_begin+X_h), padded_X_h, w, padded_X_w)] = 0;
                                }}
                            }}
                        }}
                        for (d=0; d<pad_d_end; d++) {{
                            for (h=0; h<padded_X_h; h++) {{
                                for (w=0; w<padded_X_w; w++) {{
                                    work_pad_i[mat_idx5(n, X_n, ic, aligned_X_c, (d+pad_d_begin+X_d), padded_X_d, h, padded_X_h, w, padded_X_w)] = 0;
                                }}
                            }}
                        }}
                    }}
...
#pragma omp parallel for
                    ...
                    if (W_zero_point_len == 1): # W_zero_point is scalar
                        TemplateStatements += '''
//                                                    work_pad_o[mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] += work_pad_i[mat_idx5(n, X_n, ic, X_c, current_d, padded_X_d, current_h, padded_X_h, current_w, padded_X_w)] * (W[oc][ic][kh][kw] - W_zero_point[0]);
                                                    work_pad_o[mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] += work_pad_i[mat_idx5(n, X_n, ic, X_c, current_d, padded_X_d, current_h, padded_X_h, current_w, padded_X_w)] * (*(_W_pt + mat_idx5(oc, Y_c, ic, X_c, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)) - W_zero_point[0]);
                        '''
                    else: # W_zero_point is 1D-array
                        TemplateStatements += '''
//                                                    work_pad_o[mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] += work_pad_i[mat_idx5(n, X_n, ic, X_c, current_d, padded_X_d, current_h, padded_X_h, current_w, padded_X_w)] * (W[oc][ic][kh][kw] - W_zero_point[0]);
                                                    work_pad_o[mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] += work_pad_i[mat_idx5(n, X_n, ic, X_c, current_d, padded_X_d, current_h, padded_X_h, current_w, padded_X_w)] * (*(_W_pt + mat_idx5(oc, Y_c, ic, X_c, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)) - W_zero_point[oc]);
                        '''
                    TemplateStatements += '''
                                                }}
                                            }}
...
#pragma omp parallel for
                    ...
                    if (W_zero_point_len == 1): # W_zero_point is scalar
                        TemplateStatements += '''
//                                                work_pad_o[mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] += work_pad_i[mat_idx5(n, X_n, oc, X_c, current_d, padded_X_d, current_h, padded_X_h, current_w, padded_X_w)] * (W[oc][oc/group][kh][kw] - W_zero_point[0]);
                                                work_pad_o[mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] += work_pad_i[mat_idx5(n, X_n, oc, X_c, current_d, padded_X_d, current_h, padded_X_h, current_w, padded_X_w)] * (*(_W_pt + mat_idx5(oc, Y_c, oc/group, X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)) - W_zero_point[0]);
                        '''
                    else : # W_zero_point is 1D-array
                        TemplateStatements += '''
//                                                work_pad_o[mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] += work_pad_i[mat_idx5(n, X_n, oc, X_c, current_d, padded_X_d, current_h, padded_X_h, current_w, padded_X_w)] * (W[oc][oc/group][kh][kw] - W_zero_point[0]);
                                                work_pad_o[mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] += work_pad_i[mat_idx5(n, X_n, oc, X_c, current_d, padded_X_d, current_h, padded_X_h, current_w, padded_X_w)] * (*(_W_pt + mat_idx5(oc, Y_c, oc/group, X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)) - W_zero_point[oc]);
                        '''
                    TemplateStatements += '''
                                            }}
                                        }}
...
#pragma omp parallel for
                    ...
                    if (W_zero_point_len == 1): # W_zero_point is scalar
                        TemplateStatements += '''
                                                        work_pad_o[mat_idx5(n, Y_n, current_group*group+oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] += work_pad_i[mat_idx5(n, X_n, current_group*X_c/group+ic, X_c, current_d, X_d, current_h, X_h, current_w, X_w)] * (*(_W_pt + mat_idx5(current_group*Y_c/group+oc, Y_c, ic, X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)) - W_zero_point[0]);
                        '''
                    else: # W_zero_point is 1D-array
                        TemplateStatements += '''
                                                        work_pad_o[mat_idx5(n, Y_n, current_group*group+oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] += work_pad_i[mat_idx5(n, X_n, current_group*X_c/group+ic, X_c, current_d, X_d, current_h, X_h, current_w, X_w)] * (*(_W_pt + mat_idx5(current_group*Y_c/group+oc, Y_c, ic, X_c/group, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)) - W_zero_point[current_group*group+oc]);
                        '''
                    TemplateStatements += '''
                                                    }}
                                                }}
                ...
                TemplateStatements += '''
// post-process scaling and satulation
                for (n=0; n<Y_n; n++) {{
#pragma omp parallel for
                    for (oc=0; oc<Y_c; oc++) {{
                        for (d=0; d<Y_d; d++) {{
                ...
                if (W_scale_len == 1): # W_scale is scalar
                    TemplateStatements += '''
                                    *(_Y_pt + mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_o[mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] * X_scale[0] * W_scale[0] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                    '''
                else: # W_scale is 1D-array
                    TemplateStatements += '''
                                    *(_Y_pt + mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_o[mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] * X_scale[0] * W_scale[oc] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                    '''
                TemplateStatements += '''
                                }}
                            }}
...
//                                                                    * (*(_W_pt + mat_idx5(oc, Y_c, ic, X_c, kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)) - W_zero_point[0]);
                    ...
                    if (W_zero_point_len == 1): # W_zero_point is scalar
                        ...
                        TemplateStatements += '''
                                    work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w )] += - X_zero_point[0] * w_zero_point_shift
                                                                                                        - W_zero_point[oc] * x_zero_point_shift
                                                                                                        + X_zero_point[0] * W_zero_point[oc] * kernel_counter;
                        '''
                    TemplateStatements += '''
                                }}
                            }}
...
//                                Y[n][oc][h][w] = (uint8_t)round(work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w )] * X_scale[0] * W_scale[0] / Y_scale[0] + Y_zero_point[0]);
                                *(_Y_pt + mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] * X_scale[0] * W_scale[0] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                        '''
                    else: # W_zero_point is 1D-array
                        TemplateStatements += '''
//                                Y[n][oc][h][w] = (uint8_t)round(work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w )] * X_scale[0] * W_scale[oc] / Y_scale[0] + Y_zero_point[0]);
                                *(_Y_pt + mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] * X_scale[0] * W_scale[oc] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                        '''
                    TemplateStatements += '''
                            }}
                        }}
...
#pragma omp parallel for
                    ...
                    if (W_scale_len == 1): # W_scale is scalar
                        TemplateStatements += '''
//                                Y[n][oc][h][w] = convinteger_CLAMP(round(work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w )] * X_scale[0] * W_scale[0] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                                *(_Y_pt + mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] * X_scale[0] * W_scale[0] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                        '''
                    else: # W_zero_point is 1D-array
                        TemplateStatements += '''
//                                Y[n][oc][h][w] = convinteger_CLAMP(round(work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w )] * X_scale[0] * W_scale[oc] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                                *(_Y_pt + mat_idx5(n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] * X_scale[0] * W_scale[oc] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                        '''
                    TemplateStatements += '''
                            }}
...
#pragma omp parallel for
                for (int current_group=0; current_group<group; current_group++) {{
                                        ...
                                        for (kd=kernel_shape_d_min; kd<kernel_shape_d_max; kd++) {{
                                            ...
                                            for (kh=kernel_shape_h_min; kh<kernel_shape_h_max; kh++) {{
                                                ...
                                                for (kw=kernel_shape_w_min; kw<kernel_shape_w_max; kw++) {{
                                                    current_w = w*stride_w+kw*dilation_w-pad_w_begin;
                                                    if (current_w<0 || current_w>=X_w) {{ continue; }}
                                                    work_pad_int[mat_idx5( n, Y_n, current_group*Y_c/group+oc, Y_c, d, Y_d, h, Y_h, w, Y_w )] += (*(_X_pt + mat_idx5(n, X_n, current_group*X_c/group+ic, X_c, current_d, X_d, current_h, X_h, current_w, X_w)))
                                                                        * (*(_W_pt + mat_idx5(current_group*Y_c/group+oc, Y_c, ic, (X_c/group), kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)));
                                                    x_zero_point_shift += (*(_X_pt + mat_idx5(n, X_n, (current_group*X_c/group+ic), X_c, current_d, X_d, current_h, X_h, current_w, X_w)));
                                                    w_zero_point_shift += (*(_W_pt + mat_idx5((current_group*Y_c/group+oc), Y_c, ic, (X_c/group), kd, kernel_shape_d, kh, kernel_shape_h, kw, kernel_shape_w)));
                                                    kernel_counter++;
                                                }}
                                            }}
                                        }}
                    ...
                    if (W_zero_point_len == 1): # W_zero_point is scalar
                        TemplateStatements += '''
                                        work_pad_int[mat_idx5( n, Y_n, current_group*Y_c/group+oc, Y_c, d, Y_d, h, Y_h, w, Y_w )] += - X_zero_point[0] * w_zero_point_shift
                                                                                                                                     - W_zero_point[0] * x_zero_point_shift
                                                                                                                                     + X_zero_point[0] * W_zero_point[0] * kernel_counter;
                        '''
                    else: # W_zero_point is 1D-array
                    ...
                    if (W_scale_len == 1): # W_scale is scalar
                        TemplateStatements += '''
//                                        Y[n][oc][h][w] = convinteger_CLAMP(round(work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w )] * X_scale[0] * W_scale[0] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                                        *(_Y_pt + mat_idx5(n, Y_n, current_group*Y_c/group+oc, Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_int[mat_idx5( n, Y_n, current_group*Y_c/group+oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] * X_scale[0] * W_scale[0] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                        '''
                    else: # W_zero_point is 1D-array
                        TemplateStatements += '''
//                                        Y[n][oc][h][w] = convinteger_CLAMP(round(work_pad_int[mat_idx5( n, Y_n, oc, Y_c, d, Y_d, h, Y_h, w, Y_w )] * X_scale[0] * W_scale[oc] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                                        *(_Y_pt + mat_idx5(n, Y_n, current_group*Y_c/group+oc, Y_c, d, Y_d, h, Y_h, w, Y_w)) = convinteger_CLAMP(round(work_pad_int[mat_idx5( n, Y_n, current_group*Y_c/group+oc, Y_c, d, Y_d, h, Y_h, w, Y_w)] * X_scale[0] * W_scale[oc] / Y_scale[0] + Y_zero_point[0]),{Y_min},{Y_max});
                        '''
                    TemplateStatements += '''
                                    }}
                                }}
...
# common mapping
        ...
        mapping.update({'X_scale': X_scale})
        mapping.update({'X_zero_point': X_zero_point})
        ...
        mapping.update({'W_scale_len': W_scale_len})
        mapping.update({'W_zero_point_len': W_zero_point_len})
        mapping.update({'W_scale_elements': '{'+ '1,'*(W_dm-1) +'1}'})
        mapping.update({'W_zero_point_elements': '{'+ '0,'*(W_dm-1) +'0}'})
        mapping.update({'Y_dn': Y_dn})
        mapping.update({'Y_dc': Y_dc})
        mapping.update({'Y_dd': Y_dd})
        mapping.update({'Y_dh': Y_dh})
        mapping.update({'Y_dw': Y_dw})
        mapping.update({'Y_scale': Y_scale})
        mapping.update({'Y_zero_point': Y_zero_point})
        ...
        mapping.update({'t_in': data_type.np2c(self.input_tensor_dtypes[0])})
        mapping.update({'t_out': data_type.np2c(self.output_tensor_dtypes[0])})
        mapping.update({'Y_min': -32768})
        mapping.update({'Y_max':  32767})
...
#undef TRANSPOSE
            '''
        if (len(self.input_tensor) == 4):    # zero_point x2
            TemplateFunction += cleandoc('''
            void {op_func_name}(void *op_param, {t_in} X{dims_X}, {t_in} W{dims_W}, {t_in} X_zero_point[], {t_in} W_zero_point{dims_W_zero_point}, {t_out} Y{dims}, void *inputs_params, void* outputs_params)
            {{
                {statements}
            }}
            ''')
        elif (len(self.input_tensor) == 3):    # zero_point
            TemplateFunction += cleandoc('''
            void {op_func_name}(void *op_param, {t_in} X{dims_X}, {t_in} W{dims_W}, {t_in} X_zero_point[], {t_out} Y{dims}, void *inputs_params, void* outputs_params)
            {{
                {statements}
            }}
            ''')
            ...
            TemplateFunction += cleandoc('''
            void {op_func_name}(void *op_param, {t_in} X{dims_X}, {t_in} W{dims_W}, {t_out} Y{dims}, void *inputs_params, void* outputs_params)
            {{
                {statements}
            }}
            ''')
        ...
        mappingf.update({'dims_W_scale': 1})
        mappingf.update({'dims_W_zero_point': c_helper.generate_dim_bracket(self.input_tensor_shapes[2]) if (len(self.input_tensor_shapes)>2) else 1 })
