...
class Sum(ArithmeticMixin, BackendHandler):
    ...
    def generate_c_code(self, **kwargs):
        ...
        mapping.update({'outputVal': list(output_vals.keys())[0] + list(output_vals.values())[0]})
        mapping.update({'sum_expression': '+'.join([k + v for k, v in input_vals.items()])})
        ...
        mappingf.update({'statements': TemplateArrayAddLoop.replace('[statements]', SubStatement)})
        res += '\n\n'
