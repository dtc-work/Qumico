...
class ReduceMax(ReductionMixin, BackendHandler):
    ...
    def generate_c_code_reduce(self):
        ...
        return TemplateArrayLoop.replace('[statements]',  ''.join(indent) + 'output' + output_dim + '=' + 'fmaxf(output' + output_dim + ', input' + input_dim + ');')
    ...
    def generate_c_code(self, **kwargs):
        ...
        mappingf.update({'init_statements':self.generate_c_code_init()})
