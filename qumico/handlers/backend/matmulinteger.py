...
class MatMulInteger(BackendHandler):
    ...
    def instantiate(cls, node, **kwargs):
        if (len(node.input_tensor)==2 or len(node.input_tensor)==4) : 
            pass
        else :
            raise ValueError()
    ...
    def get_param_type_name(cls):
        return 'MatMulIntegerOpParam'


    @classmethod
    def get_c_op_file_name(cls):
        return ['matmulinteger.c']
    ...
    def generate_c_code(self, **kwargs):
        ...
        if (len(self.input_tensor)==2):
            ...
            zero_point_shapes = [self.input_tensor_shapes[2][0],self.input_tensor_shapes[3][0]]
...
#pragma omp parallel for private(m,n,k,tmpA,tmpB) reduction(+:tmpY)
        ...
        if (len(self.input_tensor)==4):
            ...
            mapping.update({'tbz': data_type.np2c(self.input_tensor_dtypes[3])})
        ...
        if (len(self.input_tensor)==4):
            mappingf.update({'dims_az': c_helper.generate_dim_bracket(input_shapes[2])})
            mappingf.update({'dims_bz': c_helper.generate_dim_bracket(input_shapes[3])})
            mappingf.update({'taz': data_type.np2c(self.input_tensor_dtypes[2])})
            mappingf.update({'tbz': data_type.np2c(self.input_tensor_dtypes[3])})
