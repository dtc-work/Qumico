...
class Sigmoid(BackendHandler):
    ...
    def generate_c_code(self, **kwargs):
        ...
        # 1
        TemplateArraySigmoidLoop = c_helper.generate_ndim_for_loop(np.ones(self.output_tensor_shapes[0]),
                                                                   pragma=self.OpenMP)

        if self.OpenMP:
            TemplateArraySigmoidLoop = TemplateArraySigmoidLoop.replace('[pragma]', self.PRAGMA_OMP)

        # 2
        TemplateStatements = '''
            Y{dims} = 1/(1+exp(-X{dims}));
        '''
        ...
        mappingf.update({'statements': TemplateArraySigmoidLoop.replace('[statements]', TemplateStatements.format(**mapping))})
        res += '\n\n'
