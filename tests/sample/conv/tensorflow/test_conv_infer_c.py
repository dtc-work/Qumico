...
from samples.conv.tensorflow import conv_infer_c
...
class TestConvInferC(unittest.TestCase):
    ...
    @classmethod
    def _generate_c(cls):
        Qumico().conv_onnx_to_c(cls.model_path, cls.output_path)

    def test_init_no_input_info(self):
        self.assertRaises(AttributeError, lambda: conv_infer_c.init(so_lib_path=None,
                                                                    input_info=None,
                                                                    output_info=None))
    ...
    def test_init(self):
        dll = conv_infer_c.init(so_lib_path=self.so_lib_path, input_info=self.input_info, output_info=self.output_info)

        self.assertEqual(type(dll), ctypes.CDLL)
        self.assertTrue(hasattr(dll, "qumico"))

    def test_infer_c_no_dll(self):
        self.assertRaises(AttributeError, lambda: conv_infer_c.infer_c(dll=None, input=None, output=None))

    def test_infer_c_no_input(self):
        dll = conv_infer_c.init(so_lib_path=self.so_lib_path, input_info=self.input_info, output_info=self.output_info)
        self.assertRaises(ctypes.ArgumentError, lambda: conv_infer_c.infer_c(dll=dll, input=None, output=None))

    def test_infer_c_no_output(self):
        dll = conv_infer_c.init(so_lib_path=self.so_lib_path, input_info=self.input_info, output_info=self.output_info)
        self.assertRaises(ctypes.ArgumentError, lambda: conv_infer_c.infer_c(dll=dll,
                                                                             input=self.dll_input,
                                                                             output=None))

    def test_infer_c(self):
        dll = conv_infer_c.init(self.so_lib_path, self.input_info, self.output_info)
        res = []

        for i in prepare_infer_dataset():
            output = numpy.zeros(dtype=numpy.float32, shape=(1, 10))
            conv_infer_c.infer_c(dll, numpy.expand_dims(i, 0).astype(numpy.float32), output)
