...
from samples.automl_dogcat.tensorflow.automl_common import WIDTH, HEIGHT, LABELS
...
def infer(tflite_model_path, image_path):
    ...
    print("predictions", LABELS[label_index], predictions)


if __name__ == "__main__":
    model_path = os.path.join(os.path.dirname(__file__), "model", "model.tflite")
    image_path = os.path.join(os.path.dirname(__file__), "images", "dog1.jpeg")

    infer(tflite_model_path=model_path, image_path=image_path)
